...
class Alipay{
    ...
    public function __construct($payment_info=array(),$type='pc'){
    ...
    public function get_payurl(){
    	if($this->type=='pc'){
	    	$this->parameter = array(
	            ...
	            'partner'		    => $this->payment['partner'],	//合作伙伴ID
	            'key'               => $this->payment['key'],
	            '_input_charset'	=> strtolower('utf-8'),		//网站编码
	            'notify_url'	    => $this->payment['notify_url'],	//通知URL
	            
	            'sign_type'		    => 'MD5',	//签名方式
	            'return_url'	    => $this->payment['return_url'],	//返回URL   
	              
	            'subject'		    => $this->payment['subject'],	//商品名称       
	            'out_trade_no'	    => $this->payment['pay_order_no'],		//外部交易编号
	            'payment_type'	    => 1,			//支付类型
	            'logistics_type'    => 'EXPRESS',	//物流配送方式：POST(平邮)、EMS(EMS)、EXPRESS(其他快递)
	            'logistics_payment'	=> 'BUYER_PAY',	 //物流费用付款方式：SELLER_PAY(卖家支付)、BUYER_PAY(买家支付)、BUYER_PAY_AFTER_RECEIVE(货到付款)
	           	'logistics_fee'		=> '0.00',
	            'receive_name'		=> $this->payment['name'],//收货人姓名
	            'seller_email'		=> $this->payment['account'],	//卖家邮箱
	            'price'             => $this->payment['pay_total'],//订单总价
	            'quantity'          => 1,//商品数量
			...
			"seller_id"     => $this->payment['partner'],
			'key'               => $this->payment['key'],
			'sign_type'		    => 'MD5',
			"payment_type"	=> $this->payment['payment_type'],
			"notify_url"	=> $this->payment['notify_url'],
			"return_url"	=> $this->payment['return_url'],
			"_input_charset"	=> $this->payment['_input_charset'],
			"out_trade_no"	=> $this->payment['out_trade_no'],
			"subject"	=> $this->payment['subject'],
			"total_fee"	=> $this->payment['total_fee'],
			//"show_url"	=> 'http://pay.com/info/3',
			"show_url"	=> '',
	...
	public function notify_verify() {
		$param	= $_POST;
		$param['key']	= $this->payment['key'];
		$veryfy_url = $this->alipay_verify_url. "partner=" .$this->payment['partner']. "&notify_id=".$param["notify_id"];
