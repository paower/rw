<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:44:"./themes/default/member/transport\index.html";i:1561701883;s:34:"./shop/admin/view/public/base.html";i:1562121180;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
...
<div class="page-header">
	...
	<div class="page-header">
	<a href="<?php echo url('Transport/add'); ?>" class="btn btn-primary">新增</a>
...
<div class="row">
	<div class="col-xs-12">	
		...
		<div class="table-responsive">
			...
			<?php if (!empty($output['list'])){if(is_array($output['list']) || $output['list'] instanceof \think\Collection || $output['list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $output['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;$unit=get_weight_name(Config('weight_id')); ?>	
			<table class="table table-striped table-bordered table-hover">
				<thead>
				  ...
				  <tbody>
				    ...
				    <?php if (isset($output['extend'])&&is_array($output['extend'][$v['id']]['data'])){foreach ($output['extend'][$v['id']]['data'] as $value){?>
				    <tr>
				      <td class="bdl"></td>
...
$(function(){	
	$('a[class="J_Delete ncsc-btn-mini"]').click(function(){
		...
		if(f){
			$(this).attr('href','<?php echo url("Transport/del"); ?>'+'/id/'+id);
			return true;	
		}else{
			return false;
		}
	...
	$('a[class="J_Modify ncsc-btn-mini"]').click(function(){
		var id = $(this).attr('data-id');
		if(typeof(id) == 'undefined') return false;
		$(this).attr('href','<?php echo url("Transport/edit"); ?>'+'/id/'+id);
		return true;
	});
	...
	$('a[class="J_Clone ncsc-btn-mini"]').click(function(){
		var id = $(this).attr('data-id');
		if(typeof(id) == 'undefined') return false;
		$(this).attr('href','<?php echo url("Transport/clone_data"); ?>'+'/id/'+id);
		return true;
	});
